...
import oms.mmc.android.fast.framwork.widget.pulltorefresh.ILoadingLayout.State;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private void smoothScrollTo(int newScrollValue, long duration, long delayMillis) {
        if (mSmoothScrollAnimator != null && mSmoothScrollAnimator.isRunning()) {
        ...
        if (post) {
            mSmoothScrollAnimator = ValueAnimator.ofInt(oldScrollValue, newScrollValue);
            mSmoothScrollAnimator.setDuration(duration);
            mSmoothScrollAnimator.setInterpolator(new DecelerateInterpolator());
            mSmoothScrollAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
        ...
        if (post) {
            if (delayMillis > 0) {
                mSmoothScrollAnimator.setStartDelay(delayMillis);
            } else {
                mSmoothScrollAnimator.setStartDelay(0);
            }
